...
class RepairsService {
    ...
    async getRepairsByDateRange(startDate, endDate) {
        try {
            // Llama al modelo para filtrar por rango de fechas
            const repairs = await this.repairsModel.findRepairsByDateRange(startDate, endDate);
            return repairs;
        } catch (error) {
            throw error;
        }
